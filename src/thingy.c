...
unsigned char topicbuf[100];
char* topic = (char *)&topicbuf; 
unsigned char payloadbuf[200];
char *payload =  (char *) &payloadbuf;
char *message;
unsigned char clientid[80];
unsigned char macstr[100];
        ...
        uint16_t length, void *user_data)
{
    ...
    //Aruba Specific 
    sprintf(topic,"Thingy52/%llx/sensorinfo",cli->uuid);
    json_object_set_new( root, "tpid", json_integer(cli->tpid) );
    json_object_set_new( root, "appid", json_integer(cli->uuid) );
    json_object_set_new( root, "tstamp", json_integer(timestamp));
    json_object_set_new( root, "message", json_string(topic));
    ...
    if (value_handle == 0x001f) /* temperature */
    {
        ...
        json_object_set_new( data, "s_id", json_string("s_1") );
        json_object_set_new( data, "attr", json_string("temp") );
        json_object_set_new( data, "d1", json_integer((int)value[0]) );
        json_object_set_new( data, "d2", json_integer((int)value[1]) );
    }
        ...
        json_object_set_new( data, "s_id", json_string("s_2") );
        json_object_set_new( data, "attr", json_string("pressure") );
        json_object_set_new( data, "d1", json_integer((int)hpa) );
        json_object_set_new( data, "d2", json_integer((int)value[4]) );
    }
    else if (value_handle == 0x0025) { /* humidity */
        //    printf("Notification: Humidity received: %d \n", (int)value[0]);
        json_object_set_new( data, "s_id", json_string("s_3") );
        json_object_set_new( data, "attr", json_string("humidity") );
        json_object_set_new( data, "d1", json_integer((int)value[0]) );
    }
        ...
        json_object_set_new( data, "s_id", json_string("s_4") );
        json_object_set_new( data, "attr", json_string("co2 tvoc") );
        json_object_set_new( data, "d1", json_integer(eco2) );
        json_object_set_new( data, "d2", json_integer(tvoc) );
    }
    else {
        int i;
    ...
    json_object_set_new( data, "tpid", json_integer(cli->tpid) );
    json_object_set_new( data, "appid", json_integer(cli->uuid) );
    json_object_set_new( data, "tstamp", json_integer(timestamp));
    json_object_set_new( root,"load",data);
    ...
    printf("\n Len: %d, str:%s",strlen(payload),(char*)(pubmsg.payload));

    rc = MQTTPublish(cli->client, topic, &pubmsg);
    printf("\n status %d",rc);
...
{
    (char*)"thingy52", 0, (char*)"\n", QOS2, NULL, NULL, (char*)"localhost", 1883, 0, 4
};

void mqttcreate( struct client *cli)
{
    int rc = 0;
    json_t *root = json_object();
    json_t *data = json_object();
    ...
    MQTTClientInit(&c, &n, 1000, buf, 400, readbuf, 400);
    cli->client = &c;
    cli->nw = &n;
    cli->uuid = getuuid();
    cli->tpid= getserialno();
    sprintf(clientid,"%s%llx",opts.clientid,cli->uuid);
    cdata.willFlag = 0;
    cdata.MQTTVersion = opts.MQTTVersion;
    cdata.clientID.cstring = clientid;
    cdata.username.cstring = opts.username;
    cdata.password.cstring = opts.password;
    cdata.keepAliveInterval = 20;
    cdata.cleansession = 1;

    rc = MQTTConnect(&c, &cdata);

    //Publish a hello-registration message
    /* init */
    memset(&pubmsg, '\0', sizeof(pubmsg));
    pubmsg.qos = 0;
    pubmsg.retained = 0;
    ...
    //Aruba Specific 
    sprintf(topic,"Thingy52/registration");
    json_object_set_new( root, "tpid", json_integer(cli->tpid) );
    json_object_set_new( root, "appid", json_integer(cli->uuid) );
    json_object_set_new( root, "tstamp", json_integer(timestamp));
    json_object_set_new( root, "message", json_string(topic));
    ...
    //Application specific
    json_object_set_new( data, "attr", json_string("registration"));
    json_object_set_new( data, "tpid", json_integer(cli->tpid) );
    json_object_set_new( data, "appid", json_integer(cli->uuid) );
    json_object_set_new( data, "tstamp", json_integer(timestamp));
    json_object_set_new( data, "ver", json_integer(APP_VER) );
    json_object_set_new( data, "add",  json_string("any additonal data") );
    json_object_set_new( root,"load",data);
...
static void client_destroy(struct client *cli)
{
    ...
    bt_att_unref(cli->att);
    MQTTDisconnect(cli->client);
    NetworkDisconnect(cli->nw);
